...
Updated 2026-09-19 after the 3x-ui write-outcome and full-client-merge changes.
...
5. PostgreSQL is the commercial record for users, plans, subscriptions, wallet balance, transactions, top-ups, purchase requests, settings, and test usage. A subscription stores the 3x-ui client email/UUID/subscription ID plus its commercial state.
6. Test and wallet-paid subscription creation validates a DB plan, builds a client payload, and calls 3x-ui `clients/add` with selected inbounds. Add/update/delete and attach/bulk writes expose typed success, definitive failure, or unknown outcomes; timeout writes are never returned as `nil` and are verified when possible by reading the client back by email. Broad updates fetch the full current client before merging bot-owned fields so unrelated panel configuration survives.
7. Service management reads DB subscriptions and compares them with 3x-ui client list/traffic data. Toggles, renames, IP-limit changes, and extensions update the remote client and/or DB record through the handler flow; subscription rows remain auditable on lifecycle changes.
8. The API wrapper uses 3x-ui endpoints for inbound options, client add/update/delete, attach/detach and bulk operations, client list/traffic, and subscription links. The admin client-creation flow can create multiple customer clients and then persist their subscriptions.

## Baseline test inventory

- `go test -count=1 -v ./...` — **PASS, exit 0**; 50 passing Go test events, 7 skipped, 0 failures.
