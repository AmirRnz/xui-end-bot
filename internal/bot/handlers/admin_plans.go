...
func HandleAdminPlans(c telebot.Context) error {
	...
	for _, p := range paidPlans {
		...
		text.WriteString(fmt.Sprintf("%s #%d %s (قیمت پایه %s، %d-%d IP همزمان)\n",
...
func showAdminDraftTestPlanMenu(c telebot.Context, draft map[string]interface{}) error {
	...
	ipLabel := "نامحدود"
	if ipLimit > 0 {
		ipLabel = fmt.Sprintf("%d IP همزمان", ipLimit)
	}
...
func HandleAdminDraftEdit(c telebot.Context) error {
	...
	case "ip_limits":
		prompt = "🌐 محدودیت IP همزمان را به صورت 'پایه-حداکثر' وارد کنید (مثال: '1-6' یا '2-2')، یا عدد '0' برای نامحدود:"
	case "ip_limit":
		prompt = "🌐 محدودیت IP همزمان را وارد کنید (مثال: '1' یا '2')، یا '0' برای نامحدود:"
	case "extra_ip":
		prompt = "💲 قیمت هر IP همزمان اضافی را به تومان ارسال کنید (مثال: 50000):"
	case "discounts":
...
func ProcessAdminDraftInput(c telebot.Context, text string) error {
	...
	case "extra_ip":
		val, err := strconv.ParseInt(text, 10, 64)
		if err != nil || val < 0 {
			return c.Send("قیمت هر IP همزمان اضافی باید صفر یا یک عدد مثبت (تومان) باشد:")
		}
...
func showAdminViewPlan(c telebot.Context, planType string, planID int64) error {
	...
	if planType == db.PlanTypeTest {
		...
		ipLabel := "نامحدود"
		if plan.IPLimit > 0 {
			ipLabel = fmt.Sprintf("%d IP همزمان", plan.IPLimit)
		}
