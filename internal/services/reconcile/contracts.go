...
const (
	...
	KindSubscriptionClaimAdoption        = "subscription_claim_adoption"
)
...
type SubscriptionClaimPayload struct {
	PurchaseRequestID int64  `json:"purchase_request_id"`
	UserID            int64  `json:"user_id"`
...
func (p *SubscriptionClaimPayload) Validate() error {
	if p.PurchaseRequestID <= 0 || p.UserID <= 0 || p.PlanID <= 0 || strings.TrimSpace(p.ClientEmail) == "" || strings.TrimSpace(p.SubID) == "" {
		return errors.New("claim requires purchase request, user, plan, email, and SubID")
	}
	return nil
}

func (p *SubscriptionClaimPayload) ToMap() map[string]any { return toMap(p) }
...
func NewSubscriptionClaimRecord(p *SubscriptionClaimPayload, reqID int64) *db.ReconciliationRecord {
	return &db.ReconciliationRecord{
		OperationKey:      fmt.Sprintf("subscription_claim:%d", reqID),
		Kind:              KindSubscriptionClaimAdoption,
		UserID:            &p.UserID,
		PurchaseRequestID: &reqID,
		DesiredState:      p.ToMap(),
		ObservedState:     map[string]any{"phase": "ready"},
		Status:            "pending",
	}
}
...
func DecodeSubscriptionClaim(raw map[string]any) (*SubscriptionClaimPayload, error) {
	p := &SubscriptionClaimPayload{}
	if raw != nil {
		p.PurchaseRequestID, _ = coerceInt64(raw["purchase_request_id"])
		p.UserID, _ = coerceInt64(raw["user_id"])
		p.PlanID, _ = coerceInt64(raw["plan_id"])
		p.ClientEmail = coerceString(raw["client_email"])
		p.SubID = coerceString(raw["sub_id"])
	}
	if err := p.Validate(); err != nil {
		return nil, fmt.Errorf("invalid subscription claim payload: %w", err)
	}
	return p, nil
}
