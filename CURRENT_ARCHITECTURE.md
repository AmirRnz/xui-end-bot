...
8. **Admin Reconciliation UI**: Telegram admin interface with inspection (`admin_reconcile_detail`), immediate retry (`admin_reconcile_retry`), manual review flag (`admin_reconcile_mark_manual`), and auditable manual close requiring reason input (`admin_reconcile_close`).
