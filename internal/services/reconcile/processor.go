...
func (p *Processor) processRecord(ctx context.Context, rec *db.ReconciliationRecord) {
	...
	case KindSubscriptionClaimAdoption:
		outcome = p.handleSubscriptionClaimAdoption(ctx, rec)
	default:
...
func (p *Processor) handlePurchaseReconciliation(ctx context.Context, rec *db.ReconciliationRecord) ProcessOutcome {
	...
	case RemoteConfirmedAbsent:
		...
		return ProcessOutcome{
			Kind:   OutcomeManualReview,
			Reason: "wallet purchase create was previously attempted but the remote client is currently absent; financial and panel state require manual verification",
		}
...
func (p *Processor) startWalletPurchaseProvisioning(ctx context.Context, rec *db.ReconciliationRecord, payload *PurchaseProvisioningPayload) ProcessOutcome {
	...
	if write.Outcome != xui.WriteSucceeded {
		...
		return ProcessOutcome{Kind: OutcomeRetry, Err: fmt.Errorf("wallet purchase create did not complete: %w", write.Err)}
	}

	remote, err := p.XUI.GetClientByEmail(payload.Email)
	if ClassifyRemoteClient(remote, err) != RemoteConfirmedPresent {
		return ProcessOutcome{Kind: OutcomeRetry, Err: fmt.Errorf("created wallet client could not be verified: %w", err)}
	}
	...
	sub := &db.Subscription{
		...
		ClientEmail: payload.Email, ClientUUID: clientUUID, SubID: payload.ExpectedSubID,
...
func (p *Processor) handleSubscriptionClaimAdoption(ctx context.Context, rec *db.ReconciliationRecord) ProcessOutcome {
	payload, err := DecodeSubscriptionClaim(rec.DesiredState)
	if err != nil {
		return ProcessOutcome{Kind: OutcomeManualReview, Reason: err.Error()}
	}
	req, err := p.getPurchaseRequestByID(ctx, payload.PurchaseRequestID)
	if err != nil {
		return ProcessOutcome{Kind: OutcomeRetry, Err: fmt.Errorf("failed to load claim request: %w", err)}
	}
	if req == nil || req.Type != "claim" || req.UserID != payload.UserID || req.PlanID == nil || *req.PlanID != payload.PlanID || req.CustomName != payload.SubID {
		return ProcessOutcome{Kind: OutcomeManualReview, Reason: "claim work does not match its approved durable request"}
	}
	if req.Status != "approved" {
		if req.Status == "rejected" || req.Status == "cancelled" {
			return ProcessOutcome{Kind: OutcomeResolved, TargetStatus: db.ReconciliationStatusSuperseded, Resolution: "claim request is no longer approved"}
		}
		return ProcessOutcome{Kind: OutcomeManualReview, Reason: "claim request has not been approved"}
	}
	finder, ok := p.XUI.(interface {
		FindClientBySubID(string) (*xui.XUIClientInfo, error)
	})
	if !ok {
		return ProcessOutcome{Kind: OutcomeRetry, Err: errors.New("XUI client does not support SubID lookup")}
	}
	remote, err := finder.FindClientBySubID(payload.SubID)
	if err != nil {
		if xui.IsNotFound(err) {
			return ProcessOutcome{Kind: OutcomeManualReview, Reason: "legacy claimed client is absent from the panel"}
		}
		return ProcessOutcome{Kind: OutcomeRetry, Err: fmt.Errorf("failed to verify claimed client: %w", err)}
	}
	if remote == nil {
		return ProcessOutcome{Kind: OutcomeManualReview, Reason: "panel returned no client for the claimed SubID"}
	}
	if remote.SubID != payload.SubID || remote.Email != payload.ClientEmail || strings.TrimSpace(remote.UUID) == "" {
		return ProcessOutcome{Kind: OutcomeManualReview, Reason: "claimed client email, UUID, or SubID cannot be proven"}
	}
	existing, err := p.getSubscriptionByEmail(ctx, remote.Email)
	if err != nil {
		return ProcessOutcome{Kind: OutcomeRetry, Err: fmt.Errorf("failed to check claimed subscription: %w", err)}
	}
	if existing != nil {
		if existing.UserID != payload.UserID || existing.ClientUUID != remote.UUID || existing.SubID != remote.SubID {
			return ProcessOutcome{Kind: OutcomeManualReview, Reason: "claimed service is already linked with a different commercial identity"}
		}
	} else {
		planID := int(payload.PlanID)
		expiry := remote.ExpiryTime
		sub := &db.Subscription{
			UserID: payload.UserID, PlanID: &planID, ClientEmail: remote.Email,
			ClientUUID: remote.UUID, SubID: remote.SubID, Status: db.SubscriptionStatusActive,
			PlanType: db.PlanTypePaid, DisplayName: remote.Email, IPLimit: remote.LimitIP,
			ExpireTime: &expiry, IsActive: remote.Enable, StartDate: time.Now().UTC(),
			TrafficLimitBytes: remote.TotalGB,
		}
		if expiry > 0 {
			sub.EndDate = time.UnixMilli(expiry)
		}
		if err := db.CreateSubscription(ctx, sub); err != nil {
			return ProcessOutcome{Kind: OutcomeRetry, Err: fmt.Errorf("failed to adopt legacy claimed service: %w", err)}
		}
	}
	if err := db.SetPurchaseProvisioningStatus(ctx, payload.PurchaseRequestID, db.PurchaseProvisioningSucceeded); err != nil {
		return ProcessOutcome{Kind: OutcomeRetry, Err: fmt.Errorf("failed to mark claim adoption complete: %w", err)}
	}
	return ProcessOutcome{Kind: OutcomeResolved, TargetStatus: db.ReconciliationStatusResolvedVerified, Resolution: "legacy claimed service verified and linked"}
}
