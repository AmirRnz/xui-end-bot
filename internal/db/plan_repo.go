...
func GetPaidPlansForUser(ctx context.Context, userID int64, includeDisabled bool) ([]*PaidPlan, error) {
	...
	query := `
		SELECT p.id, p.name, p.inbound_ids, p.base_price, p.base_ip_limit, p.max_ip_limit, p.price_per_extra_ip, p.flow, p.discount_tiers, p.is_global, p.enabled, p.sync_subs, p.is_limited, p.price_per_gb, p.min_data_gb, p.price_per_extra_month, p.created_at, p.updated_at
		FROM paid_plans p
		WHERE ($2 OR p.enabled)
		  AND (
			p.is_global
...
func GetPaidPlanByID(ctx context.Context, id int64) (*PaidPlan, error) {
	...
	row := Pool.QueryRow(ctx, `
		SELECT id, name, inbound_ids, base_price, base_ip_limit, max_ip_limit, price_per_extra_ip, flow, discount_tiers, is_global, enabled, sync_subs, is_limited, price_per_gb, min_data_gb, price_per_extra_month, created_at, updated_at
...
func CreatePaidPlan(ctx context.Context, p *PaidPlan) error {
	...
	query := `
		INSERT INTO paid_plans (name, inbound_ids, base_price, base_ip_limit, max_ip_limit, price_per_extra_ip, flow, discount_tiers, is_global, enabled, sync_subs, is_limited, price_per_gb, min_data_gb, price_per_extra_month)
		VALUES ($1, $2::jsonb, $3, $4, $5, $6, $7, $8::jsonb, $9, $10, $11, $12, $13, $14, $15)
		RETURNING id, created_at, updated_at
	`
	return Pool.QueryRow(ctx, query, p.Name, string(inboundJSON), p.BasePrice, p.BaseIPLimit, p.MaxIPLimit, p.PricePerExtraIP, p.Flow, string(tierJSON), p.IsGlobal, p.Enabled, p.SyncSubs, p.IsLimited, p.PricePerGB, p.MinDataGB, p.PricePerExtraMonth).
		Scan(&p.ID, &p.CreatedAt, &p.UpdatedAt)
}
...
func UpdatePaidPlan(ctx context.Context, p *PaidPlan) error {
	...
	_, err = Pool.Exec(ctx, `
		UPDATE paid_plans
		SET name = $1, inbound_ids = $2::jsonb, base_price = $3, base_ip_limit = $4, max_ip_limit = $5,
			price_per_extra_ip = $6, flow = $7, discount_tiers = $8::jsonb, is_global = $9, enabled = $10, sync_subs = $11,
			is_limited = $12, price_per_gb = $13, min_data_gb = $14, price_per_extra_month = $15, updated_at = NOW()
		WHERE id = $16
	`, p.Name, string(inboundJSON), p.BasePrice, p.BaseIPLimit, p.MaxIPLimit, p.PricePerExtraIP, p.Flow, string(tierJSON), p.IsGlobal, p.Enabled, p.SyncSubs, p.IsLimited, p.PricePerGB, p.MinDataGB, p.PricePerExtraMonth, p.ID)
	return err
}
...
func scanPaidPlanRows(rows pgx.Rows) (*PaidPlan, error) {
	...
	p := &PaidPlan{}
	err := rows.Scan(&p.ID, &p.Name, &inboundJSON, &p.BasePrice, &p.BaseIPLimit, &p.MaxIPLimit, &p.PricePerExtraIP, &p.Flow, &tierJSON, &p.IsGlobal, &p.Enabled, &p.SyncSubs, &p.IsLimited, &p.PricePerGB, &p.MinDataGB, &p.PricePerExtraMonth, &p.CreatedAt, &p.UpdatedAt)
	if err != nil {
		return nil, err
	}
	if len(inboundJSON) > 0 {
...
func scanPaidPlanRow(row pgx.Row) (*PaidPlan, error) {
	var inboundJSON, tierJSON []byte
	p := &PaidPlan{}
	err := row.Scan(&p.ID, &p.Name, &inboundJSON, &p.BasePrice, &p.BaseIPLimit, &p.MaxIPLimit, &p.PricePerExtraIP, &p.Flow, &tierJSON, &p.IsGlobal, &p.Enabled, &p.SyncSubs, &p.IsLimited, &p.PricePerGB, &p.MinDataGB, &p.PricePerExtraMonth, &p.CreatedAt, &p.UpdatedAt)
	if err != nil {
		return nil, err
	}
	if len(inboundJSON) > 0 {
