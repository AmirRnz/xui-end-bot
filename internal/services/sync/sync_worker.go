...
func (w *SyncWorker) Start(ctx context.Context) {
	go func() {
		...
		// Run one sync on startup after brief initial delay
		select {
		case <-ctx.Done():
			return
...
func (w *SyncWorker) RunSync(ctx context.Context) {
	...
	// 1. First-connection activation sync
	...
	for _, sub := range targets {
		...
		// Check if first-connection activation occurred (remote expiry converted to positive Unix timestamp)
