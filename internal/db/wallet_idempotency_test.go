...
func TestWalletOperationKeyAppliesOneDebitAndAllowsLaterIntent(t *testing.T) {
	...
	if err := DebitWalletBalanceWithKey(ctx, userID, 100, "same wallet intent", operationKey); !errors.Is(err, ErrWalletOperationAlreadyApplied) {
...
func TestCreditWalletOperationKeyRejectsDifferentEconomicEffect(t *testing.T) {
	...
	if err := CreditWalletBalanceWithKey(ctx, userID, 2000, "subscription refund", opKey); !errors.Is(err, ErrWalletOperationConflict) {
