...
func HandleAdminSettings(c telebot.Context) error {
	...
	menu.Inline(
		...
		menu.Row(menu.Data("👥 Group Name", "admin_set_group_name"), menu.Data("🌐 IP Limit Settings", "admin_set_ip_limit_factor")),
...
func HandleAdminSyncAllIPLimitsConfirm(c telebot.Context) error {
	...
	if len(syncErrors) > 0 {
		report.WriteString("\n⚠️ **Errors:**\n")
		// Limit error count to avoid telegram message size limit
