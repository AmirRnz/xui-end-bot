...
Updated 2026-09-23. This note describes the legacy bot that is currently in this repository. It does not describe the separate backend or web panel.

## Runtime authority

- PostgreSQL is authoritative for users, orders, payment intents, integer-Toman quotes, wallet transactions, subscriptions, notifications, and reconciliation work.
- XUI is remote infrastructure state. Email alone is not enough to adopt a paid client; automatic adoption verifies the exact email, pre-persisted UUID, and SubID, then checks the durable desired state.
