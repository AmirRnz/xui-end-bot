...
func HandleAdminPlans(c telebot.Context) error {
	...
	for _, p := range paidPlans {
		...
		text.WriteString(fmt.Sprintf("%s #%d %s (قیمت پایه %s تومان، %d-%d کاربر همزمان)\n",
			enabledMark, p.ID, p.Name, persian.FormatMoney(int64(p.BasePrice)), p.BaseIPLimit, p.MaxIPLimit))
	}
...
func showAdminDraftTestPlanMenu(c telebot.Context, draft map[string]interface{}) error {
	...
	ipLabel := "نامحدود"
	if ipLimit > 0 {
		ipLabel = fmt.Sprintf("%d کاربر همزمان", ipLimit)
	}

	text := fmt.Sprintf("🧪 **تنظیمات پیش‌نویس طرح تست**\n\n"+
		"📝 نام طرح: %s\n"+
		"📝 توضیحات: %s\n"+
		"📝 راهنمای اتصال: %s\n"+
		"📡 اینباندها: %s\n"+
	...
	menu := &telebot.ReplyMarkup{}
	menu.Inline(
		menu.Row(menu.Data("📝 نام", "admin_draft_edit", "test:name"), menu.Data("📝 توضیحات", "admin_draft_edit", "test:description"), menu.Data("📝 راهنما", "admin_draft_edit", "test:usage_description")),
		menu.Row(menu.Data("📡 اینباندها", "admin_draft_inbounds", "test"), menu.Data("⏱️ مدت زمان", "admin_draft_edit", "test:duration")),
		menu.Row(menu.Data("💾 سقف حجم", "admin_draft_edit", "test:max_data"), menu.Data("⚡ فلو", "admin_draft_edit", "test:flow")),
		menu.Row(menu.Data("📊 سقف/روز", "admin_draft_edit", "test:max_per_day"), menu.Data("👥 دسترسی", "admin_draft_edit", "test:access")),
		menu.Row(menu.Data("🌐 محدودیت آی‌پی", "admin_draft_edit", "test:ip_limit"), menu.Data("🔄 همگام‌سازی: "+toggleEmoji(syncSubs), "admin_draft_toggle_sync", "test")),
		menu.Row(menu.Data("💾 ذخیره", "admin_draft_action", "test:save"), menu.Data("❌ انصراف", "admin_draft_action", "test:cancel")),
	)
...
func showAdminDraftPaidPlanMenu(c telebot.Context, draft map[string]interface{}) error {
	...
	if baseIP == 0 && maxIP == 0 {
		...
		ipLabel = fmt.Sprintf("پایه %d - حداکثر %d", baseIP, maxIP)
	}

	planTypeFa := "نامحدود"
	if isLimited {
		planTypeFa = "حجمی (محدود)"
	}

	text := fmt.Sprintf("💼 **تنظیمات پیش‌نویس طرح خرید**\n\n"+
		"📝 نام طرح: %s\n"+
		"📝 توضیحات: %s\n"+
		"📝 راهنمای اتصال: %s\n"+
		"📡 اینباندها: %s\n"+
	...
	menu := &telebot.ReplyMarkup{}
	var rows []telebot.Row
	rows = append(rows, menu.Row(menu.Data("📝 نام", "admin_draft_edit", "paid:name"), menu.Data("📝 توضیحات", "admin_draft_edit", "paid:description"), menu.Data("📝 راهنما", "admin_draft_edit", "paid:usage_description")))
	...
	if isLimited {
		...
		rows = append(rows, menu.Row(menu.Data("⏱️ قیمت هر ماه اضافه", "admin_draft_edit", "paid:price_per_extra_month"), menu.Data("⚡ فلو", "admin_draft_edit", "paid:flow")))
	} else {
		rows = append(rows, menu.Row(menu.Data("💵 قیمت پایه", "admin_draft_edit", "paid:price"), menu.Data("⚡ فلو", "admin_draft_edit", "paid:flow")))
	}

	rows = append(rows, menu.Row(menu.Data("🌐 محدودیت آی‌پی", "admin_draft_edit", "paid:ip_limits"), menu.Data("💲 قیمت آی‌پی اضافه", "admin_draft_edit", "paid:extra_ip")))
	...
	rows = append(rows, menu.Row(menu.Data("💾 ذخیره", "admin_draft_action", "paid:save"), menu.Data("❌ انصراف", "admin_draft_action", "paid:cancel")))
...
func HandleAdminDraftToggleLimited(c telebot.Context) error {
	...
	if state.Step == "awaiting_admin_draft_input" {
		return c.Send("لطفاً ابتدا ورودی متنی فعلی را تکمیل یا لغو (/cancel) کنید.")
	}
...
func HandleAdminDraftEdit(c telebot.Context) error {
	...
	if editingField == "flow" {
		...
		menu.Inline(
			...
			menu.Row(menu.Data("✏️ ورود دستی / سفارشی", "admin_draft_set_flow", editingType+":custom")),
	...
	switch editingField {
	case "name":
		prompt = "📝 نام طرح را ارسال کنید (مثال: استاندارد یک‌ماهه):"
	case "description":
		prompt = "📝 توضیحات طرح را ارسال کنید (مثال: سرور اختصاصی آلمان):"
	case "usage_description":
		prompt = "📝 راهنمای اتصال و استفاده پس از خرید را ارسال کنید:"
	case "price":
		prompt = "💵 قیمت پایه طرح را به تومان ارسال کنید (مثال: 200000):"
	case "price_per_gb":
		prompt = "💵 قیمت هر گیگابایت ترافیک را به تومان ارسال کنید (مثال: 10000):"
	case "min_data_gb":
		prompt = "💾 حداقل حجم ترافیک (به گیگابایت) را ارسال کنید (مثال: 10):"
	case "price_per_extra_month":
		prompt = "⏱️ قیمت هر ماه دوره اضافی را به تومان ارسال کنید (مثال: 20000):"
	case "ip_limits":
		prompt = "🌐 محدودیت کاربر همزمان را به صورت 'پایه-حداکثر' وارد کنید (مثال: '1-6' یا '2-2')، یا عدد '0' برای نامحدود:"
	case "ip_limit":
		prompt = "🌐 محدودیت کاربر همزمان را وارد کنید (مثال: '1' یا '2')، یا '0' برای نامحدود:"
	case "extra_ip":
		prompt = "💲 قیمت هر کاربر همزمان اضافی را به تومان ارسال کنید (مثال: 50000):"
	case "discounts":
		prompt = "🏷️ تخفیف‌ها را به فرمت 'ماه:درصد,ماه:درصد' وارد کنید (مثال: '3:10,6:20') یا '-' برای بدون تخفیف:"
	case "access":
		prompt = "👥 شناسه‌های عددی تلگرام کاربران مجاز را با کاما جدا کنید، یا '-' برای دسترسی همگانی:"
	case "duration":
		prompt = "⏱️ مدت اعتبار را به ساعت ارسال کنید (مثال: '24' یا '0.5' برای ۳۰ دقیقه):"
	case "max_data":
		prompt = "💾 سقف حجم مجاز را به گیگابایت ارسال کنید (مثال: '50') یا '0' برای نامحدود:"
	case "max_per_day":
		prompt = "📊 سقف مجاز دریافت تست روزانه برای هر کاربر را وارد کنید (مثال: '2'):"
	default:
		return c.Send("فیلد ویرایش نامشخص است.")
	}

	prompt += "\n\nبرای انصراف و بازگشت عبارت /cancel را ارسال کنید."
...
func ProcessAdminDraftInput(c telebot.Context, text string) error {
	...
	case "name":
		if text == "" {
			return c.Send("نام طرح نمی‌تواند خالی باشد. لطفاً نام معتبری وارد کنید:")
		}
		draft["name"] = text
	case "description":
	...
	case "min_data_gb":
		...
		if err != nil || val <= 0 {
			return c.Send("حداقل حجم باید یک عدد صحیح مثبت بر حسب گیگابایت باشد:")
		}
	...
	case "ip_limits":
		...
		if text == "0" || strings.ToLower(text) == "unlimited" {
			...
			if len(parts) == 1 {
				...
				if err != nil || val < 0 {
					return c.Send("محدودیت نامعتبر است. یک عدد مثبت، 0 برای نامحدود یا به فرمت پایه-حداکثر وارد کنید:")
				}
				baseIP = val
				maxIP = val
			} else if len(parts) == 2 {
				...
				if err != nil {
					return c.Send("محدودیت پایه آی‌پی نامعتبر است. دوباره تلاش کنید:")
				}
				maxIP, err = strconv.Atoi(strings.TrimSpace(parts[1]))
				if err != nil {
					return c.Send("حداکثر محدودیت آی‌پی نامعتبر است. دوباره تلاش کنید:")
				}
			} else {
				return c.Send("محدودیت آی‌پی باید یک عدد (مثلاً '2' یا '0' برای نامحدود) یا به فرمت 'پایه-حداکثر' (مثلاً '1-6') باشد:")
			}
		}

		if baseIP < 0 || maxIP < 0 {
			return c.Send("محدودیت آی‌پی نمی‌تواند منفی باشد:")
		}
		if baseIP == 0 && maxIP != 0 {
			return c.Send("در صورتی که آی‌پی پایه 0 (نامحدود) باشد، حداکثر نیز باید 0 باشد:")
		}
		if baseIP > 0 && maxIP < baseIP {
			return c.Send("حداکثر آی‌پی باید بزرگتر یا مساوی آی‌پی پایه باشد:")
		}
	...
	case "ip_limit":
		...
		if text == "0" || strings.ToLower(text) == "unlimited" {
			...
			if err != nil || val < 0 {
				return c.Send("محدودیت آی‌پی باید یک عدد صحیح مثبت یا 0 باشد:")
			}
		}
		draft["ip_limit"] = val
	case "extra_ip":
	...
	case "discounts":
		if text == "-" {
			...
			if err != nil {
				return c.Send("فرمت تخفیف‌ها نامعتبر است. از فرمت 'ماه:درصد,ماه:درصد' (مثال: '3:10,6:20') یا '-' استفاده کنید:")
			}
	...
	case "access":
		if text == "-" || text == "" {
			...
			if err != nil {
				return c.Send("شناسه‌های کاربری نامعتبر است. شناسه‌های عددی را با کاما جدا کنید یا '-' را برای دسترسی همگانی بفرستید:")
			}
	...
	case "max_per_day":
		val, err := strconv.Atoi(text)
		if err != nil || val < 0 {
			return c.Send("سقف تست روزانه باید صفر یا یک عدد صحیح مثبت باشد:")
		}
...
func HandleAdminDraftToggleSync(c telebot.Context) error {
	...
	if state.Step == "awaiting_admin_draft_input" {
		return c.Send("لطفا پیش از استفاده از دکمه‌ها، ورودی متنی فعلی را تکمیل یا لغو کنید.")
	}
...
func HandleAdminDraftSetFlow(c telebot.Context) error {
	...
	parts := strings.Split(payload, ":")
	if len(parts) != 2 {
		return c.Send("داده نامعتبر است.")
	}
	...
	if draft == nil {
		return c.Send("خطا در بارگذاری پیش‌نویس طرح.")
	}
	...
	case "custom":
		...
		bot.FSM.SetState(user.TelegramID, "awaiting_admin_draft_input", inputStateData)
		prompt := "⚡ مقدار فلو دلخواه را ارسال کنید (مثال: 'xtls-rprx-vision-less'):\n\nبرای انصراف /cancel را ارسال کنید."
		return maybeEditOrSend(c, prompt)
	case "back":
		// Do nothing
	}
...
func HandleAdminDraftAction(c telebot.Context) error {
	...
	if action == "save" {
		name := draftGetString(draft, "name")
		if name == "" {
			_ = c.Respond(&telebot.CallbackResponse{Text: "⚠️ نام طرح الزامی است."})
			return c.Send("نام طرح الزامی است. لطفاً پیش از ذخیره آن را وارد کنید.")
		}

		inboundIDs := draftGetIntSlice(draft, "inbound_ids")
		if len(inboundIDs) == 0 {
			_ = c.Respond(&telebot.CallbackResponse{Text: "⚠️ حداقل یک اینباند باید انتخاب شود."})
			return c.Send("حداقل یک اینباند باید انتخاب شود. لطفاً پیش از ذخیره اینباندها را انتخاب کنید.")
		}
		...
		if planType == "paid" {
			...
			if isLimited {
				pricePerGB := draftGetFloat64(draft, "price_per_gb")
				if pricePerGB <= 0 {
					_ = c.Respond(&telebot.CallbackResponse{Text: "⚠️ قیمت هر گیگابایت باید مثبت باشد."})
					return c.Send("قیمت هر گیگابایت باید مثبت باشد. لطفاً پیش از ذخیره آن را مشخص کنید.")
				}
				minData := draftGetInt64(draft, "min_data_gb")
				if minData <= 0 {
					_ = c.Respond(&telebot.CallbackResponse{Text: "⚠️ حداقل حجم ترافیک باید مثبت باشد."})
					return c.Send("حداقل حجم ترافیک باید مثبت باشد. لطفاً پیش از ذخیره آن را مشخص کنید.")
				}
			} else {
				basePrice := draftGetFloat64(draft, "base_price")
				if basePrice <= 0 {
					_ = c.Respond(&telebot.CallbackResponse{Text: "⚠️ قیمت پایه باید مثبت باشد."})
					return c.Send("قیمت پایه باید مثبت باشد. لطفاً پیش از ذخیره آن را مشخص کنید.")
				}
			}
		}
...
func SaveDraftPlan(c telebot.Context, planType string, draft map[string]interface{}) error {
	...
	if planType == "test" {
		...
		if id > 0 {
			...
			_ = c.Send(fmt.Sprintf("✅ طرح تست #%d با موفقیت به‌روزرسانی شد: %s", id, name))
		} else {
			...
			_ = c.Send(fmt.Sprintf("✅ طرح تست #%d با موفقیت ایجاد شد: %s", plan.ID, name))
		}
		...
		if id > 0 {
			...
			_ = c.Send(fmt.Sprintf("✅ طرح خرید #%d با موفقیت به‌روزرسانی شد: %s", id, name))
		} else {
			...
			_ = c.Send(fmt.Sprintf("✅ طرح خرید #%d با موفقیت ایجاد شد: %s", plan.ID, name))
		}
	}
...
func showAdminDraftInboundsMenu(c telebot.Context, planType string, draft map[string]interface{}) error {
	...
	var text strings.Builder
	text.WriteString("📡 **انتخاب اینباندهای متصل به طرح**\n\n")
	text.WriteString("اینباندهایی را که می‌خواهید به این طرح متصل شوند علامت بزنید:\n\n")
	text.WriteString("اینباندهای انتخاب شده:\n")

	inboundNames := cachedInboundNames()
	if len(selectedIDs) == 0 {
		text.WriteString("  _(هیچ اینباندی انتخاب نشده است)_\n")
	} else {
		for _, id := range selectedIDs {
...
func HandleAdminDraftToggleInbound(c telebot.Context) error {
	...
	bot.FSM.SetState(user.TelegramID, state.Step, draft)
	_ = c.Respond(&telebot.CallbackResponse{Text: "وضعیت اینباند تغییر کرد."})
	return showAdminDraftInboundsMenu(c, planType, draft)
}
...
func HandleAdminDraftToggleAll(c telebot.Context) error {
	...
	if action == "select" {
		...
		_ = c.Respond(&telebot.CallbackResponse{Text: "تمامی اینباندها لغو شدند."})
	}
...
func HandleAdminDraftInboundsDone(c telebot.Context) error {
	...
	_ = c.Respond(&telebot.CallbackResponse{Text: "اینباندها با موفقیت ذخیره شدند."})
...
func showAdminViewPlan(c telebot.Context, planType string, planID int64) error {
	...
	if planType == db.PlanTypeTest {
		...
		ipLabel := "نامحدود"
		if plan.IPLimit > 0 {
			ipLabel = fmt.Sprintf("%d کاربر همزمان", plan.IPLimit)
		}

		text = fmt.Sprintf("🧪 **طرح تست #%d**\n\n"+
			"📝 نام: %s\n📝 توضیحات: %s\n📝 راهنما: %s\n⚡ وضعیت فعال: %t\n👥 عمومی: %t\n"+
			"📡 اینباندها: %s\n⏱️ مدت اعتبار: %s\n💾 سقف حجم: %.2f گیگابایت\n"+
			"⚡ فلو: %s\n🌐 محدودیت آی‌پی: %s\n📊 سقف روزانه: %d\n👥 کاربران اختصاصی: %v",
		...
		text = fmt.Sprintf("💼 **طرح خرید #%d**\n\n"+
			...
			persian.FormatMoney(int64(plan.PricePerExtraIP)), plan.Flow, formatDiscountLabel(plan.DiscountTiers), access)
	}
