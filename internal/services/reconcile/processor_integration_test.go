...
func TestWalletIPUpgradeCrashAfterCommitResumesFromDurableDesiredState(t *testing.T) {
	...
	panel := &mockXUI{client: &xui.XUIClientInfo{
		Email: email, UUID: uuid, SubID: subID, Enable: true, ExpiryTime: expiry, LimitIP: 1,
	}, persistUpdate: true}
	if panel.updateCalls != 0 {
		t.Fatal("panel was changed before the simulated process-crash boundary")
	}
	// A new worker instance represents restart after the debit and durable work
	// committed but before any remote call was made.
...
func TestUnsupportedPurchaseTypeCannotBeApprovedOrMarkedSucceeded(t *testing.T) {
	...
	// Simulate an old/corrupt approved row that predates this guard. The worker
	// must stop for manual review instead of mutating XUI or marking success.
